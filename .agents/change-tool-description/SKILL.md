...
- When a description enumerates a shape that exists in code (accepted key types, object fields), build the string from the source of truth so it cannot drift: `mentionKeyShape([...])` for mention keys, `Object.keys({...} satisfies Record<keyof T, true>)` for object fields.
