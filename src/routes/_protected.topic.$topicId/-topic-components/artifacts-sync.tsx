...
import { useThreadStore } from "../../_protected.chat.$threadId/-thread-store";
...
export const ArtifactsSync = ({ topicId, threadId }: ArtifactsSyncProps) => {
  ...
  const isPolling = useThreadStore((state) =>
    state.pollingTopicIds.has(topicId)
  );
  const addPollingTopicId = useThreadStore((state) => state.addPollingTopicId);
  const removePollingTopicId = useThreadStore(
